...
import { setCookie, getCookie, delCookie, setSessionStorage, getSessionStorage, deleteSessionStorage, setEncryptedToken, getDecryptedToken, removeEncryptedToken } from '@/utils/base.js';
...
export const useUserStore = defineStore('main', {
	state: () => {
		return {
			token: getDecryptedToken() || getSessionStorage(strongToken),
			userInfo: getSessionStorage('userInfo'),
			list: {},
			memberInfo: getSessionStorage('member'),
	...
	actions: {
		...
		async afterLogin(info) {
			...
			const { data: { account, app_id, cashier_id, mobile, name, shop_supplier_id, token, user_name, update_password} } = info;
			...
			this.token = token;
			this.isDefaultPassword = update_password === 1;
			setEncryptedToken(token);

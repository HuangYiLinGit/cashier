<template>
  <el-dialog title="取单" v-model="dialogVisible" :before-close="handleClose" :close-on-click-modal="false"
    ...
    <div>
      <div v-for="(item,index) in list" :key="index" class="stay-item">
        <div class="d-b-c mb20">
          ...
          <div class="gray3 f14">挂单时间：<span class="f14 gray9">{{item.stay_time}}</span></div>
        </div>
        <el-row class="mb16" :gutter="20" v-for="(pro,proindex) in item.product" :key="proindex">
          <el-col :span="6">
            <div class="tl f14">{{pro.product.product_name}}</div>
          </el-col>
          <el-col :span="8">
            <div class="f14">￥{{pro.product_price}}</div>
          </el-col>
          <el-col :span="6">
            <div class="f14">X{{pro.product_num}}</div>
          </el-col>
        </el-row>
        <div class="d-e-c ww100">
          <el-button type="danger" size="default" @click="delStay(item)">删除</el-button>
          <el-button type="success" size="default" @click="pick(item)">取单</el-button>
        </div>
      </div>
    </div>
    <template #footer>

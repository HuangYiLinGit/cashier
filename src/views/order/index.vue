<template>
	<div class="user bg-white">
		<el-row class="user">
			<el-col :span="detail != null ? 16 : 24" class="cashier-order p20"
				:style="detail != null ? 'padding-right:0' : ''">
				<!--搜索表单-->
					...
					<el-form :inline="true" :model="searchForm" class="demo-form-inline">
						...
						<el-form-item label="下单时间">
							<el-radio-group v-model="searchForm.time_type">
								...
								<el-radio-button :value="3">一周</el-radio-button>
				...
				<div class="product-content">
					<div class="table-wrap">
						<el-tabs :model-value="searchForm.eat_type" @tab-change="orderTypeClick">
							...
							<el-tab-pane label="外卖订单" :name="'30'"></el-tab-pane>
							<el-tab-pane label="快餐订单" :name="'40'"></el-tab-pane>
						</el-tabs>
						<div class="d-s-c mb16">
							<el-button class="mr16" size="default" :type="activeName == 1 ? 'danger' : ''"
								@click="handleClick(1)" round>待完成
							</el-button>
							<el-button class="mr16" size="default" :type="activeName == 2 ? 'danger' : ''"
								@click="handleClick(2)" round>已完成
							</el-button>
							<el-button class="mr16" size="default" :type="activeName == 3 ? 'danger' : ''"
								@click="handleClick(3)" round>已取消
							</el-button>
							<el-button class="mr16" size="default" :type="activeName == 0 ? 'danger' : ''"
								@click="handleClick(0)" round>全部
							</el-button>
						</div>
						<el-table :data="tableData" style="width: 100%" v-loading="loading"
							...
							<el-table-column prop="callNo" label="名称">
								<template #default="scope">
									<div class="orange f20 fb tc line-h-84">
										{{
                      scope.row.eat_type == 10
                        ? scope.row.table_no
                        : scope.row.callNo
                    }}
									</div>
                        ...
                        : "——"
							...
							<el-table-column prop="pay_price" label="实付金额">
								<template #default="scope">
									<div class="">
										{{ scope.row.pay_price }}
										<span :class="scope.row.refund_money > 0 ? 'red' : 'green'">({{
                        scope.row.refund_money > 0
                          ? "已退款:￥" + scope.row.refund_money
                          : scope.row.pay_status.text
							...
							<el-table-column prop="callNo" label="消费方式">
								<template #default="scope">
									<div>
										<div v-if="scope.row.pay_status.value==20">
											<div class="red" v-if="scope.row.pay_type.value!=10&&scope.row.balance>0">
												<div>{{ scope.row.pay_type.text }}({{ scope.row.online_money }})</div>
												<div>余额支付({{ scope.row.balance }})</div>
											</div>
											<div class="gray9" v-else>{{ scope.row.pay_type.text }}</div>
										</div>
										<div v-else>
											<div class="gray9"></div>
										</div>
			...
			<el-col :span="detail != null ? 8 : 0" v-if="detail && detail != null" class="cashier-order-right p20">
				<el-form v-loading="loading" ref="form" :model="detail">
					...
					<el-form-item label="" :rules="[{ required: true, message: ' ' }]" prop="name">
						<div class="d-b-c pl20 ww100">
							<template v-if="detail.user">
								<div class="flex-1 f14">
									用户昵称：{{ detail.user.nickName }}
								</div>
								<div class="flex-1 f14">手机号：{{ detail.user.mobile }}</div>
							</template>
							<template v-else>
								<div class="flex-1 f14">用户昵称：无</div>
								<div class="flex-1 f14">手机号：无</div>
							</template>
						</div>
					</el-form-item>
					<div class="common-form border-t f14">订单信息</div>
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">
								订单来源：{{ detail.order_source_text }}
							</div>
							<div class="flex-1 f14">取单号：{{ detail.callNo }}</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							...
							<div class="flex-1 f14">订单号：{{ detail.order_no }}</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">订单金额：{{ detail.order_price }}</div>
							<div class="flex-1 f14">
								订单优惠：{{ detail.discount_money }}
							</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer" v-if="detail.extend_money > 0">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">
								渠道券：({{ detail.extend_name }}){{ detail.extend_code }}
							</div>
							<div class="flex-1 f14">渠道优惠：{{ detail.extend_money }}</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">实付金额：{{ detail.pay_price }}</div>
							<template v-if="detail.pay_status.value==20">
								<template v-if="detail.pay_type.value!=10&&detail.balance>0">
									<div class="flex-1 f14 red">
										支付方式：{{ detail.pay_type.text }}({{ detail.online_money }})+
										余额支付({{ detail.balance }})</div>
								</template>
								<template v-else>
									<div class="flex-1 f14">支付方式：{{ detail.pay_type.text }}</div>
								</template>
							</template>
							<template v-else>
								<div class="flex-1 f14">支付方式：</div>
							</template>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">下单时间：{{ detail.create_time }}</div>
							<div class="flex-1 f14">
								完成时间：<template v-if="detail.receipt_time">{{
                  detail.receipt_time_text
					...
					<el-form-item label="" prop="customer">
						<div class="d-b-c pl20 ww100">
							<div class="flex-1 f14">备注：{{ detail.buyer_remark }}</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer" class="pl20" max-height="300">
					...
					<el-form-item label="" pr op="customer">
						<div class="d-e-c pl20">
							<div class="mr20 f14" v-if="detail.bag_price > 0">
								包装费：{{ detail.bag_price }}元
							</div>
							<div class="mr20 f14" v-if="detail.service_money > 0">
								服务费：{{ detail.service_money }}元
							</div>
							<div class="mr20 f14" v-if="detail.refund_money > 0">
								退款金额：{{ detail.refund_money }}元
							</div>
						</div>
					</el-form-item>
					<el-form-item label="" prop="customer">
						<div class="d-e-c ww100">
							<el-button class="mr16" size="default" type="info" @click="print">打印小票</el-button>
							<!-- <el-button  class="mr16" type="primary">取单并叫号</el-button> -->
							<el-button class="mr16" size="default" type="danger" v-if="
                ...
                " @click="openRefund">
                  ...
                  (detail.eat_type == 10 || detail.eat_type == 20)
                " @click="openCashier(detail)">
								付款
							</el-button>
							<el-button class="mr16" size="default" type="success" @click="settleFunc" v-if="
                  detail.order_status.value == 10 &&
                  detail.pay_status.value == 20
                ">完成
							</el-button>
		...
		<Cash :is_pop="is_cash" :price="cartInfo.pay_price || 0" @close="closeCash"></Cash>
	</div>
</template>
<script>
		...
		methods: {
			...
			closeCash(e, form) {
				...
				if (e && e != null) {
					let Params = {
						pay_type: e,
						order_id: self.cartInfo.order_id,
						delivery: self.cartInfo.delivery_type,
						user_id: 0,
					};
					if (form) {
					...
					OrderApi.payFunc(Params, true)
						.then((res) => {
							self.awaitTime = 0;
							self.timeout = true;
							self.payStatus(self.cartInfo.order_id);
						})
						.catch((error) => {
